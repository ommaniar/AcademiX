...
class GetRecommendation:
    ...
    def __init__(self, title):
        self.post = Article.objects.get(title=title)
    ...
    def train(self):
        self.result.clear()
        other_posts = Article.objects.all().exclude(title=self.post.title)
        other_posts = other_posts.order_by('?')[:10]
        for other_post in other_posts:
            X_content = BeautifulSoup(self.post.html, 'html.parser').get_text()
            Y_content = BeautifulSoup(other_post.html, 'html.parser').get_text()
    ...
    def get_result_posts(self):
        ...
        posts = []
        for t in titles:
            posts.append(Article.objects.get(title=t))
        return posts
